...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const RoleHandler = () => {
    ...
    useEffect(() => {
      ...
      if (roleParam) {
        // If role is in URL, save it to localStorage for future use
        ...
        if (storedRole) {
          ...
          // If no role in localStorage either, redirect to login
          router.push("/")
        }
      }
    }, [searchParams, router])
  ...
  const getInitials = (role: string): string => {
    return getRoleName(role).substring(0, 2).toUpperCase()
  }
          ...
          <nav className="grid gap-1 px-2">
            {filteredNavItems.map((item, index) => (
              <Link
                ...
                className={cn(
                  "flex items-center gap-3 rounded-lg px-3 py-2 text-gray-500 transition-all hover:text-purple-900",
                  (item.href === "/dashboard" && !searchParams.get("page")) ||
                    searchParams.get("page") === item.href.split("/").pop()
